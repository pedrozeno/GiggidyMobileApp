...
public class GigListActivity extends Activity {
	...
	public final static String EXTRA_GIG_ID = "ncirl.project.giggidymobileapp.gigs.activities.GIG_ID";
		...
		lstView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				intent.putExtra(EXTRA_ARTIST_IMAGE_INFO, gm.getArtistImgURL()
						.toString());
				intent.putExtra(EXTRA_GIG_ID, gm.getGigId().toString());
	...
	private void parseJSON(JSONObject json) {
		...
		try {
			...
			for (int i = 0; i < event.length(); i++) {
				...
				model.setVenueNumber(venueObj.getString("phonenumber"));
				JSONArray venueImgArr = venueObj.getJSONArray("image");
				model.setVenueImgURL(venueImgArr.getJSONObject(
						venueImgArr.length() - 2).getString("#text"));

				JSONObject locationObj = venueObj.getJSONObject("location");
				JSONObject geoObj = locationObj.getJSONObject("geo:point");
				model.setGeoLat(geoObj.getString("geo:lat"));
				model.setGeoLong(geoObj.getString("geo:long"));

				JSONArray artistImgArr = item.getJSONArray("image");
				model.setArtistImgURL(artistImgArr.getJSONObject(
						artistImgArr.length() - 2).getString("#text"));
